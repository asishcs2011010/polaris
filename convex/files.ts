...
export const getFilePath = query({
  ...
  handler: async (ctx, args) => {
    ...
    const rootFile = await ctx.db.get("files", args.id);
    if (!rootFile) {
      throw new Error("File not found");
    }

    const project = await ctx.db.get("projects", rootFile.projectId);
    if (!project) {
      throw new Error("Project not found");
    }
    ...
    while (currentId) {
      ...
      if (visited.has(currentId)) {
        throw new Error("Detected circular parent reference in files");
      }
      visited.add(currentId);

      const currentFile: Doc<"files"> | null =
        await ctx.db.get("files", currentId);
